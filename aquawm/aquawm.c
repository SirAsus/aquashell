...
void create_blue_desktop(AquaWm *wm);
void create_orange_desktop(AquaWm *wm);
void create_red_desktop(AquaWm *wm);
void create_black_desktop(AquaWm *wm);
void load_config(AquaWm *wm);
void save_default_config(void);
char* trim_whitespace(char* str);
void reload_config(AquaWm *wm);
void* socket_thread_func(void* arg);
int setup_socket(AquaWm *wm);
void process_command(AquaWm *wm, const char* command);
...
void save_default_config(void) {
    ...
    fprintf(file, "# Edit this file to customize your window manager\n\n");
    
    fprintf(file, "# Border width around windows (0-5)\n");
    fprintf(file, "BORDER=1\n\n");
    
    fprintf(file, "# Frame width around windows (1-10)\n");
    fprintf(file, "FRAME=2\n\n");
    
    fprintf(file, "# Wallpaper type: BLUE, ORANGE, RED, BLACK\n");
    fprintf(file, "WALLPAPER=BLUE\n\n");
    
    fprintf(file, "# Gradient steps for titlebars (4-64)\n");
    fprintf(file, "GRADIENT_STEPS=16\n\n");
    
    fprintf(file, "# Show desktop grid pattern (0=no, 1=yes)\n");
    fprintf(file, "SHOW_GRID=1\n");
    
    fclose(file);
}

void load_config(AquaWm *wm) {
    wm->config.border_width = 1;
    wm->config.frame_width = 2;
    strcpy(wm->config.wallpaper, "BLUE");
    wm->config.gradient_steps = 16;
    wm->config.show_grid = 1;
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (key && value) {
            ...
            if (strcmp(trimmed_key, "BORDER") == 0) {
                ...
                if (wm->config.show_grid != 0) wm->config.show_grid = 1;
            }
        }
    }
...
void reload_config(AquaWm *wm) {
    ...
    if (strcmp(old_config.wallpaper, wm->config.wallpaper) != 0 ||
        old_config.show_grid != wm->config.show_grid) {
        create_desktop_pattern(wm);
        
        XSetWindowAttributes attrs;
        unsigned long attr_mask = CWBackPixmap;
...
void handle_reload(AquaWm *wm) {
    if (wm->reload_requested) {
        reload_config(wm);
        wm->reload_requested = 0;
    }
}
...
void process_command(AquaWm *wm, const char* command) {
    ...
    if (strcmp(command, "reload") == 0) {
        wm->reload_requested = 1;
    } else if (strcmp(command, "list") == 0) {
        printf("Active clients: %d\n", wm->num_clients);
        for (int i = 0; i < wm->num_clients; i++) {
            printf("  [%d] %s (window: %lu)\n", i, 
                   wm->clients[i]->title ? wm->clients[i]->title : "No title",
                   wm->clients[i]->window);
        }
    } else if (strncmp(command, "config ", 7) == 0) {
        char param[32];
        char value[32];
        if (sscanf(command + 7, "%s %s", param, value) == 2) {
            printf("Would set %s = %s (not implemented yet)\n", param, value);
        }
    } else {
...
void create_blue_desktop(AquaWm *wm) {
    printf("creating BLUE desktop pattern\n");
...
void create_orange_desktop(AquaWm *wm) {
    printf("creating ORANGE desktop pattern\n");
    ...
    if (wm->config.show_grid) {
        ...
        for (int y = 0; y < wm->screen_height; y += grid_size) {
            for (int x = 0; x < wm->screen_width; x += grid_size) {
                XSetForeground(wm->display, wm->desktop_gc, 0xFF8C00);
                XDrawLine(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                         x + grid_size/2, y,
                         x + grid_size, y + grid_size/2);
                XDrawLine(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                ...
                XSetForeground(wm->display, wm->desktop_gc, 0xFF4500);
                XFillArc(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                        x + 10, y + 10, 8, 8, 0, 360*64);
                XFillArc(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                        x + grid_size - 18, y + 10, 8, 8, 0, 360*64);
                XFillArc(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                        x + 10, y + grid_size - 18, 8, 8, 0, 360*64);
                XFillArc(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                        x + grid_size - 18, y + grid_size - 18, 8, 8, 0, 360*64);
            }
        }
    }
    ...
    if (font) {
        XSetFont(wm->display, wm->desktop_gc, font->fid);
        XSetForeground(wm->display, wm->desktop_gc, 0xB35900);
        XDrawString(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                   wm->screen_width - 120, wm->screen_height - 20,
                   "AquaWM", 6);
        XFreeFont(wm->display, font);
...
void create_red_desktop(AquaWm *wm) {
    printf("creating RED desktop pattern\n");
    ...
    if (wm->config.show_grid) {
        ...
        for (int y = 0; y < wm->screen_height; y += grid_size) {
            for (int x = 0; x < wm->screen_width; x += grid_size) {
                XSetForeground(wm->display, wm->desktop_gc, 0xDC143C);
                for (int i = 0; i < 4; i++) {
                    int offset = i * 15;
                    XDrawLine(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                             x + offset, y + grid_size/2,
                             x + grid_size/2, y + offset);
                    XDrawLine(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                             x + grid_size/2, y + offset,
                             x + grid_size - offset, y + grid_size/2);
                    XDrawLine(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                             x + grid_size - offset, y + grid_size/2,
                             x + grid_size/2, y + grid_size - offset);
                    XDrawLine(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                             x + grid_size/2, y + grid_size - offset,
                             x + offset, y + grid_size/2);
                }
                
                XSetForeground(wm->display, wm->desktop_gc, 0x8B0000);
                XDrawLine(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                         x + grid_size/2 - 15, y + grid_size/2,
                         x + grid_size/2 + 15, y + grid_size/2);
                XDrawLine(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                         x + grid_size/2, y + grid_size/2 - 15,
                         x + grid_size/2, y + grid_size/2 + 15);
            }
        }
    }
...
void create_black_desktop(AquaWm *wm) {
    printf("creating BLACK desktop pattern\n");
    ...
    if (wm->config.show_grid) {
        ...
        for (int y = 0; y < wm->screen_height; y += grid_size) {
            for (int x = 0; x < wm->screen_width; x += grid_size) {
                XSetForeground(wm->display, wm->desktop_gc, 0x404040);
                
                for (int size = 20; size <= 40; size += 10) {
                    XDrawRectangle(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                                  x + grid_size/2 - size/2, y + grid_size/2 - size/2,
                                  size, size);
                }
                
                XSetForeground(wm->display, wm->desktop_gc, 0x606060);
                XDrawLine(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                         x, y,
                ...
                XSetForeground(wm->display, wm->desktop_gc, 0x808080);
                XFillArc(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                        x, y, 6, 6, 0, 360*64);
                XFillArc(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                        x + grid_size - 6, y, 6, 6, 0, 360*64);
                XFillArc(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                        x, y + grid_size - 6, 6, 6, 0, 360*64);
                XFillArc(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                        x + grid_size - 6, y + grid_size - 6, 6, 6, 0, 360*64);
            }
        }
    }
    ...
    if (font) {
        XSetFont(wm->display, wm->desktop_gc, font->fid);
        XSetForeground(wm->display, wm->desktop_gc, 0x505050);
        XDrawString(wm->display, wm->desktop_pixmap, wm->desktop_gc,
                   wm->screen_width - 120, wm->screen_height - 20,
                   "AquaWM", 6);
        XFreeFont(wm->display, font);
...
void create_desktop_pattern(AquaWm *wm) {
    ...
    if (strcasecmp(wm->config.wallpaper, "ORANGE") == 0) {
        create_orange_desktop(wm);
    } else if (strcasecmp(wm->config.wallpaper, "RED") == 0) {
        create_red_desktop(wm);
    } else if (strcasecmp(wm->config.wallpaper, "BLACK") == 0) {
        create_black_desktop(wm);
    } else {
        create_blue_desktop(wm);
    }
    
    wm->desktop_drawn = 1;
    printf("Desktop pattern created successfully (%s)\n", wm->config.wallpaper);
}

AquaWm* aquawm_init(int setup_socket_server) {
    ...
    if (wm->desktop_pixmap != None) {
        ...
        unsigned long bg_color = 0x4169E1;
        if (strcasecmp(wm->config.wallpaper, "ORANGE") == 0) {
            bg_color = 0xFF8C00;
        } else if (strcasecmp(wm->config.wallpaper, "RED") == 0) {
            bg_color = 0xDC143C;
    ...
    if (setup_socket_server) {
        if (setup_socket(wm) == 0) {
    ...
    printf("config: border=%d, frame=%d, wallpaper=%s\n", 
           wm->config.border_width, wm->config.frame_width, wm->config.wallpaper);
    return wm;
}
    ...
    if (client->is_fullscreen) {
        ...
        if (client->decorated) {
            ...
            XResizeWindow(wm->display, client->window,
                         wm->workarea_width - 2 * wm->config.frame_width,
                         wm->workarea_height - TITLEBAR_HEIGHT - wm->config.frame_width);
        } else {
            XMoveResizeWindow(wm->display, client->window,
                             wm->workarea_x,
                             wm->workarea_y,
...
void toggle_maximize(AquaWm *wm, Client *client) {
    ...
    if (client->is_maximized) {
        ...
        client->x = client->frame_x + wm->config.frame_width;
        client->y = client->frame_y + TITLEBAR_HEIGHT;
        client->width = wm->workarea_width - 2 * wm->config.frame_width;
        client->height = wm->workarea_height - TITLEBAR_HEIGHT - wm->config.frame_width;
...
void update_cursor(AquaWm *wm, Window window, int x, int y) {
    ...
    int close_y = wm->config.frame_width + (TITLEBAR_HEIGHT - BUTTON_SIZE) / 2;
    int maximize_x = frame_width - wm->config.frame_width - BUTTON_SPACING - 2*BUTTON_SIZE - 2*BUTTON_SPACING;
    int maximize_y = close_y;
    int minimize_x = frame_width - wm->config.frame_width - BUTTON_SPACING - BUTTON_SIZE - BUTTON_SPACING;
    ...
    if (y < TITLEBAR_HEIGHT) {
        ...
        int edge = get_resize_edge(wm, client, x, y);
        switch (edge) {
            case 1:
            case 2:
...
void draw_window(AquaWm *wm, Client *client) {
    ...
    int gradient_steps = wm->config.gradient_steps;
    ...
    for (int i = 0; i < titlebar_gradient_steps; i++) {
        ...
        XSetForeground(dpy, gc, color);
        int y1 = wm->config.frame_width + i * (TITLEBAR_HEIGHT - wm->config.frame_width) / titlebar_gradient_steps;
        int y2 = wm->config.frame_width + (i + 1) * (TITLEBAR_HEIGHT - wm->config.frame_width) / titlebar_gradient_steps;
        XFillRectangle(dpy, win, gc, wm->config.frame_width, y1, 
                      frame_width - 2 * wm->config.frame_width, y2 - y1 + 1);
    }
...
void handle_motion_notify(AquaWm *wm, XEvent *e) {
    ...
    if (client->decorated && !client->is_fullscreen && !client->is_dock) {
        if (wm->is_moving && !client->is_maximized) {
            ...
            client->x = new_x + wm->config.frame_width;
            ...
            if (wm->resize_edge & 1) {
                ...
                if (new_x < wm->workarea_x + wm->config.frame_width) {
                    new_x = wm->workarea_x + wm->config.frame_width;
                    new_width = client->orig_x + client->orig_width - wm->workarea_x - wm->config.frame_width;
                }
            }
            if (wm->resize_edge & 2) {
                new_width = client->orig_width + dx;
                if (new_width < MIN_WIDTH) new_width = MIN_WIDTH;
                if (new_width > wm->workarea_width - (new_x - wm->workarea_x) - 2 * wm->config.frame_width)
                    new_width = wm->workarea_width - (new_x - wm->workarea_x) - 2 * wm->config.frame_width;
            }
            if (wm->resize_edge & 4) {
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1) {
        if (strcmp(argv[1], "-r") == 0) {
            printf("Sending reload command to AquaWM\n");
            if (send_command_to_wm("reload") == 0) {
                printf("Reload command sent successfully!\n");
            } else {
                printf("Failed to send reload command. Is AquaWM running?\n");
                return 1;
            }
            return 0;
            ...
            printf("Sending command '%s' to AquaWM...\n", argv[2]);
            if (send_command_to_wm(argv[2]) == 0) {
                printf("Command sent successfully\n");
            } else {
                printf("Failed to send command. Is AquaWM running?\n");
                return 1;
            }
            return 0;
        } else if (strcmp(argv[1], "-d") == 0) {
            printf("Starting AquaWM in daemon mode\n");
        }
    }
    ...
    AquaWm *wm = aquawm_init(setup_socket);
    if (!wm) {
